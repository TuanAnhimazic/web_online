<div class="">
    <div class="page-title">
        <div class="title_left">
            <h3><?= $data['title']?></h3>
                <a href="admin/addcategory" class="btn btn-primary">Add new</a>  
            ...
            <table class="table table-striped jambo_table bulk_action">
                ...
                <tbody>
                    ...
                    <tr class="even pointer">
                        ...
                        <td>
                        <a style="height: 35px;" class="btn btn-success" href="<?=base?>admin/editcategory&id=<?=$values['id']?>&page=<?=$data['currentpage']?>">Edit</a>
                            <a style="height: 35px" class="btn btn-danger submit" href="javascrip:void(0)" onclick="del(<?=$values['id']?>,'<?=$values['name'] ?>','<?=base.'admin/deletecategory/'?>','category ',<?=$data['currentpage']?>,<?=$stt?>)"  >Delete</a> 
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
    ...
    <?php if($data["currentpage"] != 1 && $data["currentpage"] >= 4){?>
        <a class="page-item" href="<?=base?>admin/showcategory&page=<?=1?>">First page</a>
        <?php }?>
    ...
    <?php if($data["currentpage"] != $data["total"] && $data["currentpage"] <= $data["total"] - 3){?>
        <a class="page-item" href="<?=base?>admin/showcategory&page=<?=$data["total"]?>">Last page</a>
        <?php }?>
</div>
